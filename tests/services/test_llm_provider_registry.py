# thirdparty
import pytest

# project
from services.config.runtime_config import ConfigValidationError, load_runtime_config
from services.llm_providers.mock_provider import MockLLMProvider
from services.llm_providers.openai_provider import OpenAILLMProvider
from services.llm_providers.registry import UnknownLLMProviderError, get_provider


def _local_env(**overrides: str) -> dict[str, str]:
    env = {
        "ENV_MODE": "local",
        "LLM_PROVIDER": "mock",
        "OPENAI_API_KEY": "sk-test",
        "DATABASE_URL": "postgresql+asyncpg://postgres:password@localhost:5433/mini_app_db_test",
        "DATABASE_URL_PSYCOPG2": "postgresql+psycopg2://postgres:password@localhost:5433/mini_app_db_test",
        "ANALYSIS_RUNTIME_ENABLED": "false",
    }
    env.update(overrides)
    return env


def test_registry_returns_openai_provider(monkeypatch):
    class _StubAsyncOpenAI:
        def __init__(self, **kwargs):
            self.kwargs = kwargs

    config = load_runtime_config(env=_local_env(LLM_PROVIDER="openai"))
    monkeypatch.setattr("services.llm_providers.registry.get_runtime_config", lambda: config)
    monkeypatch.setattr("services.llm_providers.openai_provider.AsyncOpenAI", _StubAsyncOpenAI)

    provider = get_provider("openai")
    assert isinstance(provider, OpenAILLMProvider)


def test_registry_blocks_openai_in_test_mode(monkeypatch):
    config = load_runtime_config(
        env=_local_env(
            ENV_MODE="test",
            LLM_PROVIDER="openai",
            ANALYSIS_RUNTIME_ENABLED="true",
            OPENAI_API_KEY="sk-test",
        )
    )
    monkeypatch.setattr("services.llm_providers.registry.get_runtime_config", lambda: config)

    with pytest.raises(ConfigValidationError, match="ENV_MODE=test"):
        get_provider("openai")


def test_registry_unknown_provider_raises_without_mock_fallback(monkeypatch):
    config = load_runtime_config(env=_local_env())
    monkeypatch.setattr("services.llm_providers.registry.get_runtime_config", lambda: config)

    with pytest.raises(UnknownLLMProviderError):
        get_provider("nonexistent-provider")


def test_registry_default_uses_mock_in_test_mode():
    provider = get_provider()
    assert isinstance(provider, MockLLMProvider)
